/*
  # Adicionar system_prompt à Função acquire_next_prompt_lock

  1. Alteração
    - Adiciona coluna `system_prompt` ao retorno da função acquire_next_prompt_lock
    - Necessário para que as edge functions recebam o system_prompt junto com o prompt_content

  2. Impacto
    - Edge functions poderão usar system_prompt ao chamar a LLM
    - Suporta o padrão systemInstruction do Gemini API
*/
...
COMMENT ON FUNCTION acquire_next_prompt_lock IS 'Adquire lock atômico no próximo prompt disponível para processamento. Inclui system_prompt para uso com LLMs.';