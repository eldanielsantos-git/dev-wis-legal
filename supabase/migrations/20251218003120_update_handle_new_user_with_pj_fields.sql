/*
  # Atualizar função handle_new_user para suportar campos de Pessoa Jurídica

  1. Mudanças
    - Adiciona leitura dos campos `type`, `company_name` e `cnpj` dos metadados do usuário
    - Atualiza o INSERT para incluir os novos campos na criação do perfil
    - Mantém compatibilidade com cadastros PF e PJ

  2. Campos Adicionados
    - `type`: Tipo do usuário (PF ou PJ)
    - `company_name`: Razão social da empresa (apenas para PJ)
    - `cnpj`: CNPJ da empresa (apenas para PJ)
*/
...
DECLARE
  ...
  v_cnpj TEXT;
BEGIN
  -- Obter tipo do usuário (PF ou PJ)
  v_type := NEW.raw_user_meta_data->>'type';
  ...
  -- Inserir perfil com dados extraídos (incluindo campos de PJ)
