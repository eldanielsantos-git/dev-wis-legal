/*
  # Incluir Company Name nas Notificações de Signup PJ
  
  ## Objetivo
  Atualizar o trigger de notificação de signup para incluir o nome da empresa (company_name) 
  quando o tipo de usuário for PJ (Pessoa Jurídica).
  
  ## Formato das Mensagens
  
  ### PF (Pessoa Física)
  - message: "Nome | Email | PF"
  - Preview: "Novo Usuário | Nome | Email | PF"
  
  ### PJ (Pessoa Jurídica)
  - message: "Nome | Email | PJ | Company Name"
  - Preview: "Novo Usuário | Nome | Email | PJ | Company Name"
  
  ## Exemplos
  - PF: "João Silva | joao@example.com | PF"
  - PJ: "Maria Santos | maria@example.com | PJ | Tech Solutions Ltda"
*/
...
BEGIN
  ...
  -- Nome da empresa (para PJ)
  v_company_name := COALESCE(NEW.company_name, '');
  
  -- Montar mensagem no formato correto
  IF v_user_type = 'PJ' AND v_company_name != '' THEN
    -- PJ com empresa: Nome | Email | PJ | Company Name
    v_message := v_user_name || ' | ' || v_user_email || ' | ' || v_user_type || ' | ' || v_company_name;
  ELSE
    -- PF ou PJ sem empresa: Nome | Email | Type
...
Formato PF: Nome | Email | PF
Formato PJ: Nome | Email | PJ | Company Name
Preview mobile: Novo Usuário | [mensagem formatada]';
