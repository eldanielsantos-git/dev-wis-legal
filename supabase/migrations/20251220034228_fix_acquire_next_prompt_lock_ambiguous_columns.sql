/*
  # Fix acquire_next_prompt_lock - Remove Ambiguous Column References

  ## Problem
  Column reference "id" is ambiguous in the function because both tables 
  (analysis_results and analysis_prompts) have an 'id' column.

  ## Solution
  Add explicit table aliases to all column references to remove ambiguity.

  ## Changes
  1. Drop existing function
  2. Recreate with fully qualified column names using table aliases
*/

-- Drop the existing function with ambiguous columns
DROP FUNCTION IF EXISTS acquire_next_prompt_lock(uuid, timestamptz, timestamptz);

-- Create the corrected RPC function with fully qualified columns
...
BEGIN
  ...
  WHERE ar.prompt_id = ap.id
    ...
    AND ar.id = (
      ...
      WHERE ar2.processo_id = p_processo_id
        AND ar2.status = 'pending'
      ORDER BY ar2.execution_order ASC
      LIMIT 1
    )
  RETURNING
    ar.id,
...
COMMENT ON FUNCTION acquire_next_prompt_lock IS 'Acquires lock for next pending prompt ONLY if no other prompt is currently processing. Ensures sequential execution (1→2→3→4) instead of parallel execution. Fixed: Uses fully qualified column names to avoid ambiguity.';