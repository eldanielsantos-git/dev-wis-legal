/*
  ...
  1. Problema
    - Função tenta retornar coluna system_prompt que não existe em analysis_results
    - Causa erro: "column analysis_results.system_prompt does not exist"
    
  2. Solução
    - Remover system_prompt do retorno
    - O system_prompt deve ser buscado de analysis_prompts quando necessário
    - Simplificar a função para usar apenas campos existentes
*/

-- Dropar função existente
DROP FUNCTION IF EXISTS acquire_next_prompt_lock(uuid, timestamptz, timestamptz);

-- Recriar função SEM system_prompt
