...
export async function notifyAdminSafe(params: NotifyAdminParams): Promise<void> {
  try {
    ...
    if (response.ok) {
      ...
      console.warn(`[notify-admin-safe] Falha ao enviar notificação admin: ${response.status} - ${errorText}`);
    }
  } catch (error) {
    console.warn('[notify-admin-safe] Erro ao notificar admin (não-crítico):', error instanceof Error ? error.message : 'Unknown');
  }
}