...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    console.log(`[${callId}] 🔒 Tentando adquirir lock GLOBAL do processo...`);

    const { data: lockAcquired, error: globalLockError } = await supabase.rpc('acquire_processo_lock', {
    ...
    if (globalLockError) {
      console.error(`[${callId}] ❌ Erro ao adquirir lock global:`, globalLockError);
      throw new Error('Erro ao adquirir lock global: ' + globalLockError.message);
    }

    if (!lockAcquired) {
      console.log(`[${callId}] ⏸️ Lock global ocupado por outro worker. Aguardando...`);
      return new Response(
        JSON.stringify({
          success: true,
          completed: false,
          message: 'Outro worker está processando este processo no momento',
          waiting: true
        }),
        {
          status: 200,
    ...
    console.log(`[${callId}] ✅ Lock global adquirido! Processando sequencialmente...`);
    console.log(`[${callId}] 🔒 Tentando adquirir próximo prompt para processar...`);
    ...
    if (!nextResult) {
      ...
      if (allCompleted) {
        ...
        await supabase.rpc('release_processo_lock', {
          p_processo_id: processo_id,
          p_worker_id: workerId
        });
        console.log(`[${callId}] 🔓 Lock global liberado`);
        ...
        console.log(`[${callId}] 🔓 Lock global liberado (nenhum prompt disponível)`);
    ...
    for (const model of activeModels) {
      ...
      const maxTokens = model.max_tokens ?? 60000;
      ...
      if (shouldUseFileApi) {
        ...
        try {
          ...
          console.log(`[${callId}] 🔓 Lock global liberado (File API)`);
          return new Response(
            JSON.stringify({
              success: true,
              completed: false,
        ...
        try {
          ...
          console.log(`✅ Status atualizado com sucesso (Base64):`, {
            ...
            contentLength: text.length,
            updated: !!updateData,
            returnedContentLength: updateData?.[0]?.result_content?.length || 0
          });
          ...
          if (verifyData) {
            const savedLength = verifyData.result_content?.length || 0;
            console.log(`🔍 Verificação pós-save (Base64): ${savedLength} caracteres salvos no banco`);
            if (savedLength === 0 && text.length > 0) {
              console.error(`❌ ALERTA CRÍTICO: Conteúdo foi perdido! Enviado: ${text.length}, Salvo: ${savedLength}`);
              console.error(`📝 Conteúdo original (primeiros 500 chars): ${text.substring(0, 500)}`);
            }
          }
          ...
          console.log(`[${callId}] 🔓 Lock global liberado (Base64)`);
          return new Response(
            JSON.stringify({
              success: true,
              completed: false,
    ...
    try {
      if (processo_id && typeof workerId !== 'undefined') {
        await supabase.rpc('release_processo_lock', {
          p_processo_id: processo_id,
          p_worker_id: workerId
        });
        console.log('🔓 Lock global liberado (erro)');
      }
    } catch (lockError) {
      console.error('Erro ao liberar lock:', lockError);
    }
