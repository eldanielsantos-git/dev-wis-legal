    ...
    >
      <div className="flex items-center justify-between">
        <div className="flex-1 min-w-0">
          <p className="text-xs sm:text-sm font-medium mb-1 truncate" style={{ color: theme === 'dark' ? '#8B8B8B' : '#6B7280' }}>{title}</p>
          <p className="text-xl sm:text-2xl lg:text-3xl font-bold" style={{ color: theme === 'dark' ? '#C8C8C8' : '#1F2937' }}>{value}</p>
          {description && (
            <p className="text-[10px] sm:text-xs mt-1 sm:mt-2 line-clamp-2" style={{ color: theme === 'dark' ? '#8B8B8B' : '#6B7280' }}>{description}</p>
          )}
          {trend && (
        ...
        <div className={`p-2 sm:p-3 lg:p-4 rounded-lg sm:rounded-xl ${iconBg} flex-shrink-0 ml-2`}>
