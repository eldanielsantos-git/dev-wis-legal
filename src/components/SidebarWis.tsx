  ...
  return (
    <>
      ...
      <aside className={`hidden lg:flex ${isCollapsed ? 'w-20' : 'w-64'} h-screen fixed left-0 top-0 flex-col font-body transition-[width] duration-300 ease-in-out`} style={{ backgroundColor: colors.bgPrimary, borderRight: `1px solid ${colors.border}` }}>
        <div className="p-6 max-h-900:p-4 flex flex-col items-center justify-center flex-shrink-0">
          <button
            onClick={onNavigateToApp}
            className="hover:opacity-80 transition-opacity w-10 h-10 max-h-900:w-9 max-h-900:h-9 flex items-center justify-center"
            title="Ir para página inicial"
          >
            <img
              src={colors.logo}
              alt="Wis Legal"
              className="h-10 w-10 max-h-900:h-9 max-h-900:w-9 object-contain"
            />
          </button>
        </div>

        <nav className="flex-1 px-3 max-h-900:px-2.5 flex flex-col overflow-y-auto scrollbar-thin scrollbar-thumb-gray-600 scrollbar-track-transparent">
          <div>
            <button
              onClick={handleToggleCollapse}
              className={`w-full flex items-center py-3 max-h-900:py-2 rounded-lg transition-colors mb-2 max-h-900:mb-1.5 ${isCollapsed ? 'justify-center' : 'px-4 max-h-900:px-3'}`}
              style={{ color: colors.textPrimary }}
              onMouseEnter={(e) => e.currentTarget.style.backgroundColor = hoverBg}
              onMouseLeave={(e) => e.currentTarget.style.backgroundColor = 'transparent'}
            ...
            >
              ...
              {!isCollapsed && <span className="ml-3 max-h-900:ml-2.5 text-sm font-medium">Busca</span>}
            </button>
            ...
            >
              <Plus className="w-5 h-5 flex-shrink-0" />
              {!isCollapsed && <span className="ml-3 max-h-900:ml-2.5 text-sm font-medium">Novo processo</span>}
            </button>
            ...
            >
              <FileText className="w-5 h-5 flex-shrink-0" />
              {!isCollapsed && <span className="ml-3 max-h-900:ml-2.5 text-sm font-medium">Meus processos</span>}
            </button>
            ...
            >
              <MessageSquare className="w-5 h-5 flex-shrink-0" />
              {!isCollapsed && <span className="ml-3 max-h-900:ml-2.5 text-sm font-medium">Chat</span>}
            </button>
            ...
            >
              <Users className="w-5 h-5 flex-shrink-0" />
              {!isCollapsed && <span className="ml-3 max-h-900:ml-2.5 text-sm font-medium">Meu workspace</span>}
            </button>
            ...
            >
              <Calendar className="w-5 h-5 flex-shrink-0" />
              {!isCollapsed && <span className="ml-3 max-h-900:ml-2.5 text-sm font-medium">Agenda</span>}
            </button>
          </div>

          <div className="flex-1"></div>

          <div className={`${isCollapsed ? '' : 'pt-4 max-h-900:pt-3'}`} style={!isCollapsed ? { borderTop: `1px solid ${colors.border}` } : {}}>
            <button
              onClick={toggleTheme}
              className={`w-full flex items-center py-3 max-h-900:py-2 rounded-lg transition-colors ${isCollapsed ? 'justify-center' : 'px-4 max-h-900:px-3'}`}
              style={{ color: colors.textPrimary }}
            ...
            >
              {theme === 'dark' ? <Sun className="w-5 h-5 flex-shrink-0" /> : <Moon className="w-5 h-5 flex-shrink-0" />}
              {!isCollapsed && <span className="ml-3 max-h-900:ml-2.5 text-sm font-medium">{theme === 'dark' ? 'Modo claro' : 'Modo escuro'}</span>}
            </button>
            ...
            >
              <Coins className="w-5 h-5 flex-shrink-0" />
              {!isCollapsed && <span className="ml-3 max-h-900:ml-2.5 text-sm font-medium">Tokens</span>}
            </button>
            ...
            >
              <UserPlus className="w-5 h-5 flex-shrink-0" />
              {!isCollapsed && <span className="ml-3 max-h-900:ml-2.5 text-sm font-medium">Enviar convite</span>}
            </button>

          </div>
        </nav>

        <div className="p-2 max-h-900:p-1.5 flex-shrink-0" style={!isCollapsed ? { borderTop: `1px solid ${colors.border}` } : {}}>
          {isCollapsed ? (
            <div className="flex items-center py-2 max-h-900:py-1.5 justify-center">
              <UserAvatarMenu
                avatarUrl={profile?.avatar_url}
                firstName={profile?.first_name}
                lastName={profile?.last_name}
            ...
            <div className="flex items-center py-2 max-h-900:py-1.5 px-3 max-h-900:px-2.5">
              <UserAvatarMenu
                avatarUrl={profile?.avatar_url}
                firstName={profile?.first_name}
                lastName={profile?.last_name}
