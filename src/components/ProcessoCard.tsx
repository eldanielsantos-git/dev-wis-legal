    ...
    const notifyAdminOnError = async () => {
      ...
      if (processo.status === 'error' && !errorNotifiedRef.current.has(processo.id)) {
        try {
          ...
          // Chamar edge function para verificar e notificar
          const response = await fetch(
            `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/check-failed-processes`,
            {
              method: 'POST',
              headers: {
  ...
  return (
    <div
      ...
      >
        ...
        {workspaceInfo && (
          <div
            className="mt-3 sm:mt-4 lg:max-h-[768px]:mt-2 lg:max-h-[768px]:sm:mt-2.5 pt-3 sm:pt-4 lg:max-h-[768px]:pt-2 lg:max-h-[768px]:sm:pt-2.5 border-t space-y-2 lg:max-h-[768px]:space-y-1"
            style={{ borderColor: theme === 'dark' ? 'rgba(200, 200, 200, 0.1)' : 'rgba(229, 231, 235, 0.8)' }}
          >
            {workspaceInfo.sharedWith && (
              <div className="flex items-center text-xs lg:max-h-[768px]:text-[9px]" style={{ color: theme === 'dark' ? '#8B8B8B' : '#6B7280' }}>
                <Users className="w-3.5 h-3.5 lg:max-h-[768px]:w-2.5 lg:max-h-[768px]:h-2.5 mr-2 lg:max-h-[768px]:mr-1 flex-shrink-0" />
            ...
            {workspaceInfo.sharedAt && (
              <div className="flex items-center text-xs lg:max-h-[768px]:text-[9px]" style={{ color: theme === 'dark' ? '#8B8B8B' : '#6B7280' }}>
                <Calendar className="w-3.5 h-3.5 lg:max-h-[768px]:w-2.5 lg:max-h-[768px]:h-2.5 mr-2 lg:max-h-[768px]:mr-1 flex-shrink-0" />
                <span>
            ...
            {workspaceInfo.permissionLevel && (
              workspaceInfo.onManageShares ? (
                <button
                  ...
                  className="inline-flex items-center space-x-2 lg:max-h-[768px]:space-x-1 px-3 lg:max-h-[768px]:px-2 py-1.5 lg:max-h-[768px]:py-0.5 rounded-full text-xs lg:max-h-[768px]:text-[9px] font-medium transition-all"
                  style={{
                    backgroundColor: 'rgba(59, 130, 246, 0.1)',
                    color: '#3b82f6'
                  }}
                ...
                >
                  <Users className="w-3 h-3 lg:max-h-[768px]:w-2 lg:max-h-[768px]:h-2" />
                  <span>Gerenciar compartilhamento</span>
                </button>
              ) : (
                <div
                  className="inline-flex items-center space-x-2 lg:max-h-[768px]:space-x-1 px-3 lg:max-h-[768px]:px-2 py-1.5 lg:max-h-[768px]:py-0.5 rounded-full text-xs lg:max-h-[768px]:text-[9px] font-medium"
                  style={{
                ...
                >
                  {workspaceInfo.permissionLevel === 'read_only' ? (
                    <>
                      <Lock className="w-3 h-3 lg:max-h-[768px]:w-2 lg:max-h-[768px]:h-2" />
                      <span>Somente Leitura</span>
                    </>
                  ) : (
                    <>
                      <Edit3 className="w-3 h-3 lg:max-h-[768px]:w-2 lg:max-h-[768px]:h-2" />
                      <span>Editor</span>
                    </>
                  )}
                </div>
        ...
        {(userInfo || processo.user_profile) && (
          <div
            className="mt-3 sm:mt-4 lg:max-h-[768px]:mt-2 lg:max-h-[768px]:sm:mt-2.5 pt-3 sm:pt-4 lg:max-h-[768px]:pt-2 lg:max-h-[768px]:sm:pt-2.5 border-t"
            style={{ borderColor: theme === 'dark' ? 'rgba(200, 200, 200, 0.1)' : 'rgba(229, 231, 235, 0.8)' }}
          >
            <div
              className="space-y-1.5 lg:max-h-[768px]:space-y-0.5 text-xs lg:max-h-[768px]:text-[9px]"
              style={{ color: theme === 'dark' ? '#8B8B8B' : '#6B7280' }}
            >
        ...
        {processo.status === 'completed' && (
          <div
            ...
            style={{ borderColor: theme === 'dark' ? 'rgba(200, 200, 200, 0.1)' : 'rgba(229, 231, 235, 0.8)' }}
          >
            <div className="grid grid-cols-3 gap-3 lg:max-h-[768px]:gap-1.5 text-center">
              <div>
                <p
                  className="text-lg lg:max-h-[768px]:text-sm font-bold"
                  style={{ color: theme === 'dark' ? '#FFFFFF' : '#141312' }}
                >
                  {processo.transcricao?.totalPages || 0}
                </p>
                <p
                  className="text-xs lg:max-h-[768px]:text-[9px]"
                  style={{ color: theme === 'dark' ? '#8B8B8B' : '#6B7280' }}
                >
                  Páginas
                </p>
              ...
              <div>
                <p className="text-lg lg:max-h-[768px]:text-sm font-bold">
                  <CheckCircle
                    className="w-6 h-6 lg:max-h-[768px]:w-4 lg:max-h-[768px]:h-4 inline"
                    style={{ color: theme === 'dark' ? '#FFFFFF' : '#141312' }}
                  />
                </p>
                <p
